...
### 3.1 代托管混合网关模式（试点后优化）
当前代托管模式下，所有用户的托管云盘来源都接入同一组服务器本机 AList/OpenList。业务数据已经按 account 隔离，但云盘登录、token 刷新、目录遍历和播放取流的出口仍集中在托管服务器 IP。小范围试点阶段先保持当前模式，通过限并发、错峰扫描和 provider 级节流降低风险；不要在首批试点前引入大规模架构改造。

试点稳定后再评估混合网关模式：

- PC 客户端内置 OpenList 或轻量本地网关，扫码、凭证保存、目录访问和播放取流优先走用户自己的设备和网络出口。
- 安卓客户端可采用“使用时启动本地网关”的方式，不假设长期后台常驻，避免电量、保活和系统限制问题。
- Web 端继续保留云端托管模式；纯浏览器无法直接运行本地 OpenList，除非用户安装 PC/安卓伴随程序并完成本地配对。
- 云端只保存账号、片库、影视元数据、观看记录、审查结果、挂载摘要和必要索引；云盘 refresh token 等敏感凭证优先留在用户设备。
- 如需要跨设备恢复，云端只保存加密备份，设计目标是服务端不能明文读取用户云盘凭证。
- 数据模型后续预留 `gateway_type=cloud/local`、`gateway_id`、`credential_location=cloud/local/encrypted_backup` 等概念，避免未来从纯云端托管迁移时推倒重来。

该项属于托管规模扩大后的优化方案，不是当前内测必须交付功能。
