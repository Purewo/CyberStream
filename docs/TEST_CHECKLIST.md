...
- smoke check 返回 `OK catalog_metadata_issue_filter`，说明 `metadata_issue_code` 筛选能返回带对应 issue code 的影片
- smoke check 返回 `OK catalog_metadata_source_group_filter`，说明 `metadata_source_group` 筛选能返回同 source group 的影片
...
- `metadata_fallback_pipeline_match` 返回 `OK`，说明兜底匹配审核队列数量和样本契约可用
- `pending_review_backfill_dry_run` 返回 `OK`，说明待审回填 dry-run 入口可用且不会提交发布状态
...
- smoke check 返回 `OK movie_seasons`，说明季列表入口、季级海报/资源计数和 `episode_diagnostics` 契约可用
- smoke check 返回 `OK movie_episode_diagnostics`，说明剧集诊断详情入口、dry-run 修复建议和提交 payload 契约可用
...
- smoke check 返回 `OK background_job_detail`，说明后台任务详情轮询契约可用；无任务时会安全跳过
...
2026-06-07 维护基线：`765 passed, 9 skipped, 16 subtests passed`。
