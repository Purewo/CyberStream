...
`backend_smoke_check.py` 会同时检查 `/` 与 `/api/v1/health` 健康入口一致性、OpenAPI 健康入口、文档索引、OpenAPI 模块索引、扫描状态、元数据 provider 注册表、复核工作台入口契约、其他视频归档队列、外部播放器播放交接、字幕显示设置、音频转码诊断读取、后台任务列表入口、存储源列表与资源型挂载动作、fallback/episode 队列和资源治理 live check；带 `--systemd` 时还会检查 `cyberstream-backend`、`nginx`、`cyberstream-alist`、`cyberstream-openlist` 和 `ddns-go`。
...
该检查会验证 `GET /api/v1/auth/me` 的启动认证态契约，逐个拉取 `GET /api/v1/openapi/modules/<module_key>.json`，验证元数据概览、详情页图片缓存状态、播放资源的云转码能力声明、存储 provider 类型、挂载表单字段和协议能力矩阵，并调用 `GET /api/v1/storage/sources/<id>/health` 和 `GET /api/v1/system/tmdb-config/check`。`--min-storage-health-checks 1` 用于防止实际没有检查到资源型挂载时误判通过；TMDB 检查不返回 token 明文。
...
`backup` 会先写入临时文件，自动校验通过后再发布为正式备份路径；校验失败时会尽量删除未确认的副本。需要人工复查时可再次校验：
...
恢复会先自动校验目标备份文件；校验通过后再自动创建一次恢复前备份，并替换当前数据库。恢复失败时会尽量删除临时恢复文件。必须显式确认：
