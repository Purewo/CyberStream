...
该检查会逐个拉取 `GET /api/v1/openapi/modules/<module_key>.json`，验证存储 provider 类型、挂载表单字段和协议能力矩阵，并调用 `GET /api/v1/storage/sources/<id>/health` 和 `GET /api/v1/system/tmdb-config/check`。`--min-storage-health-checks 1` 用于防止实际没有检查到资源型挂载时误判通过；TMDB 检查不返回 token 明文。
