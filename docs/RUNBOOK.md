...
该检查会验证 `GET /api/v1/auth/me` 的启动认证态契约，逐个拉取 `GET /api/v1/docs/<doc_key>` 和 `GET /api/v1/openapi/modules/<module_key>.json`，验证 `GET /api/v1/aggregator/sources` 源列表、元数据概览、TMDB 配置读取、详情页图片缓存状态、播放资源的云转码能力声明和 `streaming-qualities` 清晰度入口、`/stream` 的小 Range 响应或 3xx 跳转、外部播放器 manifest 与 `format=m3u` 文本播放列表、季列表、剧集诊断 dry-run、收藏列表/收藏状态的保险库访问契约、存储 provider 类型、挂载表单字段、已保存挂载点详情和协议能力矩阵，并调用 `GET /api/v1/storage/sources/<id>/health` 和 `GET /api/v1/system/tmdb-config/check`。`--min-storage-health-checks 1` 用于防止实际没有检查到资源型挂载时误判通过；TMDB 配置读取和 token 检查都不返回 token 明文。
