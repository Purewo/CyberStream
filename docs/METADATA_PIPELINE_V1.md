...
- 动漫库可通过 `provider_order: ["nfo", "anilist", "bangumi", "tmdb", "local"]` 显式启用动漫来源优先；默认顺序仍为 `nfo -> tmdb -> local`
...
- `GET /api/v1/metadata/providers` 返回当前 provider 注册表、默认顺序、别名和能力；当前为 `nfo/tmdb/anilist/bangumi/tencent_video/local`
...
- AniList 候选 ID 为 `anilist/<id>`，支持关键字、裸 anime ID、`anilist/<id>` 和 `https://anilist.co/anime/<id>` 定点搜索
...
- `POST /api/v1/movies/{id}/metadata/match` 建议传 `candidate_id + provider`，同时兼容 `tmdb_id`、`imdb/<id>`、`tvdb/<id>`、`movie/<id>`、`tv/<id>`、`anilist/<id>` 和 `bangumi/<id>`；默认只返回 dry-run 预览，前端确认覆盖时需带 `apply=true`
