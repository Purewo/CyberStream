...
- pytest 全量基线通过：`772 passed, 9 skipped, 16 subtests passed`
...
- 当前统一版本：`1.22.0`（以 `backend/config.py` 的 `APP_VERSION` 为单一版本源）
...
### 4.2 敏感配置已外置，仍需守住边界
`TMDB_TOKEN`、历史 WebDAV 凭证、存储凭证和会话密钥均通过环境变量或 `.env.local` 注入，代码内不再保留真实明文默认值。
...
### 4.4 历史版本管理曾不一致
该问题已完成收口，当前根路由健康检查返回版本与 `backend/config.py` 中的 `APP_VERSION` 保持一致，当前为 `1.22.0`。
...
截至 2026-06-14，`1.22.0` 已作为代托管多租户联调基线。`1.16.0` 保留为历史稳定标签。
...
- OpenAPI `1.22.0-beta` 与运行时路由对齐
- 当前自动化测试基线为 `772 passed, 9 skipped, 16 subtests passed`

当前项目暂按单主干维护，后续小步提交直接进入 `main`。

## 5.2 2026-04-28 历史接手记录
...
- 当前维护工作目录：`/root/CyberStream`
- 历史部署目录：`/home/pureworld/赛博影视`
