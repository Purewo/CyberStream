...
- 当前统一版本：`1.16.0`（以 `backend/config.py` 的 `APP_VERSION` 为单一版本源）
...
### 4.3 历史版本管理曾不一致
该问题已完成收口，当前根路由健康检查返回版本与 `backend/config.py` 中的 `APP_VERSION` 保持一致，当前为 `1.16.0`。
...
截至 2026-04-25，`1.16.0` 已作为当前稳定联调基线收口。
