...
async function fetchApi<T>(path: string, options?: RequestInit): Promise<T | null> {
  try {
    ...
    if (json.code !== 200 && json.code !== 20000) {
      console.warn(`API Business Error ${json.code}: ${json.msg || (json as any).message}`);
      return null;
    }
    return json.data;
  } catch (error) {
...
async function fetchApiRaw<T>(path: string, options?: RequestInit): Promise<RawApiResult<T>> {
  try {
    ...
    return {
      ok: res.ok && body && (body.code === 200 || body.code === 20000),
      status: res.status,
      code: body?.code,
...
const mapApiMovieToUi = (apiMovie: ApiMovieSimple | ApiMovieDetailed): Movie => {
  ...
  return {
    ...
    manual_content: apiMovie.manual_content
  };
};
