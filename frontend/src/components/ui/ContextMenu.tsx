...
export const ContextMenu: React.FC<ContextMenuProps> = ({ visible, x, y, movie, isFavorite, activeLibraryId, onClose, onAction }) => {
  ...
  if (py + 240 > window.innerHeight) {
    ...
    { id: 'add_to_library', icon: <FolderPlus size={15} />, label: '添加到专辑...', color: 'hover:text-teal-400' },
    ...(activeLibraryId ? [{ id: 'remove_from_library', icon: <FolderMinus size={15} />, label: '从当前专辑移除', color: 'hover:text-red-400' }] : []),
