...
export const ScanProgressBar = () => {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        // Support both old backend (status) and new backend (state/phase)
        let currentState = (status as any)?.state || (status as any)?.status || (status as any)?.phase;
        
        // Normalize v3.9 phase to unified states based on isScanning
        if ((status as any)?.isScanning) {
           currentState = currentState === 'stopping' ? 'stopping' : 'scanning';
        } else if (currentState === 'done' || currentState === 'error') {
           currentState = 'idle';
        }
  ...
  let rawState = scanStatus.state || (scanStatus as any).status || (scanStatus as any).phase;
  if ((scanStatus as any).isScanning) {
    rawState = rawState === 'stopping' ? 'stopping' : 'scanning';
  } else if (rawState === 'done' || rawState === 'error') {
    rawState = 'idle';
  }
  const currentState = rawState;

  const currentFile = scanStatus.current_file || (scanStatus as any).current_item || (scanStatus as any).currentFile || (scanStatus as any).phaseDetail;
  let progressPercent = scanStatus.progress ?? 0;
  
  if ((scanStatus as any).discoveredCount > 0 && (scanStatus as any).processedCount !== undefined) {
      progressPercent = Math.min(100, Math.round(((scanStatus as any).processedCount / (scanStatus as any).discoveredCount) * 100));
  }
